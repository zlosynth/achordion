...
pub fn quantize(mode: Mode, root: Note, mut voct: f32) -> Note {
    ...
    if difference < above_mean(semitones[0], semitones[1]) / 12.0 {
        ...
        to_midi_note(close_root + semitones[1] as f32 / 12.0)
    } else if difference < above_mean(semitones[2], semitones[3]) / 12.0 {
        to_midi_note(close_root + semitones[2] as f32 / 12.0)
    } else if difference < above_mean(semitones[3], semitones[4]) / 12.0 {
        to_midi_note(close_root + semitones[3] as f32 / 12.0)
    } else if difference < above_mean(semitones[4], semitones[5]) / 12.0 {
        to_midi_note(close_root + semitones[4] as f32 / 12.0)
    } else if difference < above_mean(semitones[5], semitones[6]) / 12.0 {
        to_midi_note(close_root + semitones[5] as f32 / 12.0)
    } else if difference < above_mean(semitones[6], 12) / 12.0 {
        to_midi_note(close_root + semitones[6] as f32 / 12.0)
    } else {
...
mod tests {
    ...
    fn quantize_on_spot_voct_to_note_with_root_below() {
        let voct = 2.0;
        assert_eq!(quantize(Ionian, Note::C0, voct), Note::C1);

        let voct = 2.0 + 2.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::C0, voct), Note::D1);

        let voct = 2.0 + 4.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::C0, voct), Note::E1);

        let voct = 2.0 + 5.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::C0, voct), Note::F1);

        let voct = 2.0 + 7.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::C0, voct), Note::G1);

        let voct = 2.0 + 9.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::C0, voct), Note::A1);

        let voct = 2.0 + 11.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::C0, voct), Note::B1);

        let voct = 3.0;
        assert_eq!(quantize(Ionian, Note::C0, voct), Note::C2);
    }

    #[test]
    fn quantize_on_spot_with_note_below_the_lowest_root() {
        let voct = 1.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::A0, voct), Note::CSharpMinus1);
    }

    #[test]
    fn quantize_on_spot_voct_to_note_with_root_above() {
        let voct = 2.0;
        assert_eq!(quantize(Ionian, Note::C3, voct), Note::C1);

        let voct = 2.0 + 2.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::C3, voct), Note::D1);

        let voct = 2.0 + 4.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::C3, voct), Note::E1);

        let voct = 2.0 + 5.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::C3, voct), Note::F1);

        let voct = 2.0 + 7.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::C3, voct), Note::G1);

        let voct = 2.0 + 9.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::C3, voct), Note::A1);

        let voct = 2.0 + 11.0 / 12.0;
        assert_eq!(quantize(Ionian, Note::C3, voct), Note::B1);

        let voct = 3.0;
        assert_eq!(quantize(Ionian, Note::C3, voct), Note::C2);
    }

    #[test]
    fn quantize_note_over_limit_and_stay_in_scale() {
        let voct = 100.0;
        assert_eq!(quantize(Ionian, Note::B0, voct), Note::Gb9);
    }
