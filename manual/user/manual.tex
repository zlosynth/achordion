...
While inputs TONE and TONE' follow the 1V/oct standard, they may not precisely match the connected CV sources device. To calibrate each of these inputs, follow this procedure:
...
There are two audio outputs in the module: OUT and OUT'. When only one of these outputs is connected, it gets mixed with the unconnected one. When both outputs are connected, each plays different voices.
...
Due to the headroom reserved for all 18 voices, output levels are quite weak. To compensate for that, digital overdrive can be enabled by tapping the WAVETABLE CV input with a jack while holding the button. Note that this harms sound quality.

\section{Reset}

Calibration settings and all secondary parameters (wavetable bank, scale tonic, scale mode, chord mode) are stored between restarts of the module. To reset their values, hold the button while powering on the module.
