...
Audio outputs OUT (6) and OUT' (7) play the main and the side line respectively. When only one is connected, the outputs are mixed together.
...
Due to the headroom reserved for all 18 voices, output levels are quite weak. To compensate for that, digital overdrive can be enabled by tapping the WAVETABLE CV input with a jack while holding the button. Note that this has a negative effect on sound quality.
